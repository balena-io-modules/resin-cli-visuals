...
const getAlias = (ordering: Ordering[], column: string): string | undefined =>
	_.result(
		ordering.find((o) => 'name' in o && o.name === column),
		'alias',
	);
...
const applySubtitles = function (table: string, ordering: Ordering[]) {
	...
	const titleizedTable = splitTable.map(function (row, rowIndex) {
		if (!row.startsWith('$X$')) {
			return row;
		}
...
export function horizontal(data: object[], ordering: string[] = []) {
	...
	return trimRight(
		columnify(data, {
			...
			columns: orderingObj.map((o) =>
				'name' in o ? o.name : undefined,
			) as string[],
...
export function vertical(data: object, ordering?: string[]) {
	...
	const orderingObj = parseOrdering(ordering, data).filter(
